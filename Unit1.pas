...
procedure TForm1.VisiBtnV(fl: boolean); // управление доступностью компонентов ведения списка товаров в приходном документе
begin
...
procedure TForm1.VisiBtnP(fl: boolean); // управление доступностью компонентов ведения списка товаров в приходном документе
begin
...
procedure TForm1.VisiBtnPr(fl: boolean); // управление доступностью компонентов ведения списка товаров в приходном документе
begin
...
var ssss: string;
    k: integer;
begin
ssss:= 'Вы собираетесь уволить сотрудника "'+ Qu.fieldbyName ('FIO').AsString+'"?';
...
var sss : string;
  ...
  begin
    Result:= true; sss:='';
   if length(Trim(Edit1.Text))<10 then sss:=sss+ '-Слишком короткое ФИО'+#13#10;
   if length(Trim(Edit2.Text))<5 then sss:=sss+'-Неверно введена должность'+#13#10;
   if sss<>'' then begin
 result:=false;
...
var ssss: string;
    k: integer;
begin
ssss:= 'Вы собираетесь удалить пациента "'+ Qu1.fieldbyName ('FIO').AsString+'"?';
k:= MessageBoxEx(Self.Handle, Pchar(ssss), 'Подтвердите намерения...',
   MB_YESNO or MB_ICONQUESTION or MB_DEFBUTTON2, (SUBLANG_DEFAULT shl 10) or LANG_RUSSIAN);
    If k=IDNO then Exit;
    rej:=3;
...
var sss : string;
  ...
  begin
    Result:= true; sss:='';
   if length(Trim(Edit3.Text))<10 then sss:=sss+ '-Слишком короткое ФИО'+#13#10;
   if length(Trim(Edit4.Text))<>10 then sss:=sss+'-Неверно введена дата'+#13#10;
   if sss<>'' then begin
 result:=false;
 messageboxex(self.handle, PChar(' При входном контроле введенных данных обнаружены ошибки: '+#13#10+'исправьте указанные ошибки ввода и попытайтесь снова: '+#13#10+#13#10+sss),'операция невыполнима', MB_OK or MB_ICONERROR, (SUBLANG_DEFAULT shl 10) or LANG_RUSSIAN);
 end;
...
var ssss: string;
    k: integer;
begin
ssss:= 'Вы собираетесь удалить процедуру "'+ Qu.fieldbyName ('FIO').AsString+'"?';
k:= MessageBoxEx(Self.Handle, Pchar(ssss), 'Подтвердите намерения...',
   MB_YESNO or MB_ICONQUESTION or MB_DEFBUTTON2, (SUBLANG_DEFAULT shl 10) or LANG_RUSSIAN);
    If k=IDNO then Exit;
    rej:=3;
...
var sss : string;
  ...
  begin
    Result:= true; sss:='';
   if length(Trim(Edit5.Text))<2 then sss:=sss+ '-Слишком маленькая стоимость'+#13#10;
   if length(Trim(Edit2.Text))>11 then sss:=sss+'-Неверно введена дата'+#13#10;
   if sss<>'' then begin
 result:=false;
 messageboxex(self.handle, PChar(' При входном контроле введенных данных обнаружены ошибки: '+#13#10+'исправьте указанные ошибки ввода и попытайтесь снова: '+#13#10+#13#10+sss),'операция невыполнима', MB_OK or MB_ICONERROR, (SUBLANG_DEFAULT shl 10) or LANG_RUSSIAN);
 end;
